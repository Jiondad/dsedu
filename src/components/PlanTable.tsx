          ...
          <tbody className="divide-y divide-gray-100">
            {sortedPlans.length === 0 ? (
              ...
              sortedPlans.map(({ plan, originalIndex }, idx) => {
                // 💡 [버그 완전 격파] plan_id와 planId 두 가지 조건 모두를 정밀 추적하여 완벽 방어 처리
                const hasDraft = drafts.some((d) => 
                  (d.plan_id && String(d.plan_id) === String(plan.id)) || 
                  ((d as any).planId && String((d as any).planId) === String(plan.id))
                );

                const hasReport = reports.some((r) => 
                  (r.plan_id && String(r.plan_id) === String(plan.id)) || 
                  ((r as any).planId && String((r as any).planId) === String(plan.id))
                );
