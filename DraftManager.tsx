...
export default function DraftManager({
  ...
  const validate = (): boolean => {
    ...
    if (!draftDate) {
      newErrors.draftDate = '기안일자를 선택해주세요.';
    } else if (selectedPlan) {
      const planDate = selectedPlan.date || selectedPlan.edu_date;
      if (planDate && draftDate > planDate) {
        newErrors.draftDate = `기안일자는 교육일자(${planDate})보다 늦을 수 없습니다.`;
        triggerLocalNotification(`기안일자는 교육일자(${planDate})보다 늦을 수 없습니다.`, 'error');
        try {
          alert(`[입력 오류] 기안일자(${draftDate})는 해당 교육의 교육일자(${planDate})보다 같거나 먼저(<=)여야 합니다.`);
        } catch (_) {}
      }
    }
